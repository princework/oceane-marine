...
export default function VendorRegisterPage() {
  ...
  const saveEdit = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/master/vendors/${editingId}/update`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async (id) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/master/vendors/${id}/delete`, { method: "DELETE" });
      const data = await res.json();
      ...
      <div className="mx-auto max-w-[95%] pl-3 sm:pl-4 pr-3 sm:pr-4 py-6 sm:py-10 space-y-4 sm:space-y-6">
        <header className="mt-12 md:mt-0 mb-2 flex flex-col items-center gap-4 sm:flex-row sm:items-start sm:gap-4">
          ...
          <div className="flex-1 flex flex-col items-center text-center w-full sm:w-auto">
            ...
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white">Vendor Register</h1>
            <p className="text-xs sm:text-sm text-slate-200 mt-1">
              Vendor name and email used across the qualification pipeline
            </p>
          </div>
          ...
          {vendors.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {vendors.map((v) => (
                    <tr key={v._id} className="border-b border-white/5 hover:bg-white/5 transition align-top">
                      ...
                      <td className="py-3 pr-4">
                        {editingId === v._id ? (
                          ...
                          <span className="text-slate-300">{v.email}</span>
                        )}
                      </td>
                      <td className="py-3 pr-4">
                        {editingId === v._id ? (
                          ...
                          <div className="flex items-center gap-2">
                            <ActionEditIcon onClick={() => startEdit(v)} title="Edit vendor" />
                            <ActionDeleteIcon
                              onClick={() => handleDelete(v._id)}
                              disabled={!!editingId}
                              loading={deletingId === v._id}
                              title="Delete vendor"
                            />
                          </div>
                        )}
