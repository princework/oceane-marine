...
export default function TrainingPlanReviewPage() {
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/qhse/training/plan/${planId}`);
  ...
  const handleApprove = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/qhse/training/plan/${planId}/approve`, {
        method: "PUT",
      });
  ...
  const handleRejectSubmit = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/qhse/training/plan/${planId}/reject`, {
        method: "PUT",
      ...
      <div className={`${contentClassName} w-full min-w-0 pr-4`}>
        <div className="mx-auto max-w-[95%] pl-3 sm:pl-4 pr-3 sm:pr-4 py-10">
          <div className="rounded-3xl border border-red-500/40 bg-red-950/40 p-6">
            <p className="text-red-300">{error || "Training plan not found"}</p>
            <Link
  ...
  const monthPairFiles = plan.monthPairFiles || {};
  ...
  return (
    <div className={`${contentClassName} w-full min-w-0 pr-4`}>
      <div className="mx-auto max-w-[95%] pl-3 sm:pl-4 pr-3 sm:pr-4 py-6 sm:py-6 sm:py-10 space-y-3 sm:space-y-4 sm:space-y-6">
        <header className="mt-12 md:mt-0 mb-2 flex flex-col items-center gap-4 sm:flex-row sm:items-start sm:gap-4">
          ...
          <div className="flex-1 flex flex-col items-center text-center w-full sm:w-auto">
            ...
            <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white">
              Review Training Plan — {plan.year}
            </h1>
            <p className="text-xs sm:text-sm text-slate-200 mt-1">
              Form code: <span className="font-mono font-semibold text-sky-300">{plan.formCode || "QAF-OFD-038"}</span>
        ...
        {MONTH_PAIR_LABELS.some((label) => monthPairFiles[label]) && (
          <div className="rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-6 backdrop-blur shadow-2xl">
            ...
            <ul className="space-y-2 text-sm">
              {MONTH_PAIR_LABELS.filter((label) => monthPairFiles[label]).map((label) => (
                <li key={label}>
                  <span className="text-slate-400">{label}: </span>
                  <a
                    href={`/api/qhse/file/${String(monthPairFiles[label].filePath || "").replace(/^\//, "")}`}
                    target="_blank"
                    rel="noreferrer"
                    className="text-sky-300 hover:underline"
                  >
                    {monthPairFiles[label].fileName || "Download"}
                  </a>
                </li>
              ))}
            </ul>
      ...
      {showRejectModal && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="training-plan-reject-modal-title"
        >
          <div className="bg-slate-800 border border-white/15 rounded-xl shadow-xl max-w-md w-full p-6">
            <h2 id="training-plan-reject-modal-title" className="text-lg font-semibold text-white mb-4">
              Reject Training Plan
            </h2>
            <p className="text-slate-300 text-sm mb-3">
              Year: {plan.year} • {plan.serialNumber || "—"}
            </p>
            <label htmlFor="training-plan-reject-reason" className="block text-sm text-slate-400 mb-1">
              Reason <span className="text-red-400">*</span>
            </label>
            <textarea
              id="training-plan-reject-reason"
