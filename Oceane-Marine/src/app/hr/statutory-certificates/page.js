...
import StatutoryCertificatesFormPage from "./form/StatutoryCertificatesFormPage";
import StatutoryCertificatesListPage from "./list/StatutoryCertificatesListPage";
...
function StatutoryCertificatesContent() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-white flex">
      <HrSidebarWrapper />
      <div className={`flex-1 min-w-0 overflow-auto transition-all duration-300 ${isSidebarOpen ? "ml-0 md:ml-72" : "mx-auto max-w-7xl"}`}>
        <div className="w-full mx-auto py-4 sm:py-6 md:py-10 space-y-4 sm:space-y-6 px-3 sm:px-4">
          ...
          >
            ...
            <div className="flex-1 min-w-0 flex flex-col items-center text-center w-full md:w-auto">
              <p className="text-xs sm:text-sm uppercase tracking-[0.15em] sm:tracking-[0.25em] text-sky-300">
                HR / Statutory Certificates
              </p>
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white">Statutory Certificates</h1>
            </div>
          ...
          {activeTab === "form" && (
            <StatutoryCertificatesFormPage 
              onSuccess={() => {
                setActiveTab("list");
                // Refresh list if it's already loaded
                if (typeof window !== "undefined") {
                  window.dispatchEvent(new Event("refreshCertificates"));
                }
              }} 
            />
          )}
          {activeTab === "list" && <StatutoryCertificatesListPage onRefresh={() => setActiveTab("list")} />}
        </div>
      </div>
    </div>
  );
...
export default function StatutoryCertificatesPage() {
  return (
    <Suspense
      fallback={
