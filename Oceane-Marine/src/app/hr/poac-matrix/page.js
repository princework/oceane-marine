...
import PoacMatrixFormPage from "./form/PoacMatrixFormPage";
import PoacMatrixListPage from "./list/PoacMatrixListPage";
          ...
          >
            ...
            <div className="flex-1 min-w-0 flex flex-col items-center text-center w-full md:w-auto">
              <p className="text-xs sm:text-sm uppercase tracking-[0.15em] sm:tracking-[0.25em] text-sky-300">
                HR / POAC Certification Matrix
              </p>
              <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white">POAC Certification Matrix</h1>
            </div>
            ...
            <PoacMatrixFormPage 
              onSuccess={() => {
                ...
                if (typeof window !== "undefined") {
                  window.dispatchEvent(new Event("refreshPoacMatrix"));
                }
              }} 
            />
          )}
          {activeTab === "list" && <PoacMatrixListPage onRefresh={() => setActiveTab("list")} />}
        </div>
      </div>
    </div>
  );
...
export default function PoacMatrixPage() {
  return (
    <Suspense
      fallback={
