  ...
  try {
    ...
    if (!operationRef) {
      return NextResponse.json(
        { error: "operationRef query parameter is required" },
        { status: 400, headers: CORS_HEADERS }
      );
    }

    let expenseSheet = await MooringMasterExpenseSheet.findOne({ operationRef })
      .sort({ createdAt: -1 })
      .lean();

    if (!expenseSheet) {
      // Try case-insensitive
      expenseSheet = await MooringMasterExpenseSheet.findOne({
        operationRef: { $regex: new RegExp(`^${operationRef}$`, "i") },
      })
        .sort({ createdAt: -1 })
        .lean();
    }
    ...
    if (!expenseSheet) {
      return NextResponse.json(
        { error: "Expense sheet not found for this operationRef" },
    ...
    console.error("OPS-OFD-029 GET error:", error);
    return NextResponse.json(
      { error: error.message },
...
export async function PUT(req) {
  ...
  try {
    ...
    let existing = await MooringMasterExpenseSheet.findOne({ operationRef })
      .sort({ createdAt: -1 })
      .lean();

    if (!existing) {
      existing = await MooringMasterExpenseSheet.findOne({
        operationRef: { $regex: new RegExp(`^${operationRef}$`, "i") },
      })
        .sort({ createdAt: -1 })
        .lean();
    ...
    void notifyOperationsEdit("OPS-OFD-029", updatedExpenseSheet._id);
    return NextResponse.json(
      {
    ...
    console.error("OPS-OFD-029 PUT error:", error);
    return NextResponse.json(
      { error: error.message },
      { status: 500, headers: CORS_HEADERS }
    );
