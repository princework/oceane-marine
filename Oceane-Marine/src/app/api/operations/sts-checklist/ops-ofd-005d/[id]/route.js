...
/**
 * GET /api/operations/sts-checklist/ops-ofd-005d/[id]
...
export async function GET(req, { params }) {
  ...
  try {
    ...
    const checklist = await STSChecklistFiveF.findById(id).lean();
...
/**
 * PUT /api/operations/sts-checklist/ops-ofd-005d/[id]
...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    // Handle signature uploads for all three signature blocks
    const terminalBerthedShipSignatureUrl = await handleSignatureUpload(
      formData,
      body,
      "terminalBerthedShipSignature",
      existing.terminalBerthedShipSignature?.signature
    );
    const outerShipSignatureUrl = await handleSignatureUpload(
      formData,
      body,
      "outerShipSignature",
      existing.outerShipSignature?.signature
    );
    const terminalSignatureUrl = await handleSignatureUpload(
      formData,
      body,
      "terminalSignature",
    ...
    const updateData = {
      ...
      checklistItems,
      terminalBerthedShipSignature: mapSignatureBlock(
        body,
        "terminalBerthedShipSignature",
        terminalBerthedShipSignatureUrl,
        existing.terminalBerthedShipSignature || {}
      ),
      outerShipSignature: mapSignatureBlock(
        body,
        "outerShipSignature",
        outerShipSignatureUrl,
        existing.outerShipSignature || {}
      ),
      terminalSignature: mapSignatureBlock(
        body,
        "terminalSignature",
        terminalSignatureUrl,
    ...
    void notifyOperationsEdit("OPS-OFD-005D", id);
    return NextResponse.json(
      {
        success: true,
