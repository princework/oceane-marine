...
export async function GET(req) {
  ...
  try {
    ...
    let checklists = await STSChecklistFiveF.find(query)
      .populate("createdBy", "name email")
      .sort({ createdAt: -1 })
      .lean();

    console.log(`📋 Found ${checklists.length} OPS-OFD-005D checklists for year ${year || 'all'}`);
    ...
    const allChecklists = await STSChecklistFiveF.find({})
      .select("revisionDate createdAt")
      .lean();

    const yearsSet = new Set();
    allChecklists.forEach((checklist) => {
      // Check revisionDate first
      if (checklist.revisionDate) {
