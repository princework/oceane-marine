import ShipStandardQuestionnaire from "../../lib/mongodb/models/operation-sts-checklist/OPS-OFD-001-A.js";
import Operation from "../../lib/mongodb/models/sts-documentation/StsOperation.js";
import { generateOpsOfd001ADoc } from "../services/pdf/OPS-OFD-001A.js";
import { getPdfStoragePath } from "../services/pdf/pdfStorage.js";
import fs from "node:fs";
import path from "node:path";

const DOCUMENT_TYPE = "OPS-OFD-001A";

export function defineOpsOfd001AJob(agenda) {
    agenda.define("generate-ops-ofd-001a", async (job) => {
        ...
        try {
            console.log("🚀 Starting OPS-OFD-001A document generation job", { checklistId, operationRef });
            ...
            // ==================== FETCH QUESTIONNAIRE ====================
            const questionnaire = await ShipStandardQuestionnaire.findById(checklistId).lean();
            if (!questionnaire) {
                throw new Error(`Questionnaire not found with ID: ${checklistId}`);
            }

            if (!questionnaire.sequenceNumber) {
                throw new Error("Questionnaire missing sequenceNumber");
            }

            console.log(`📄 Generating document with Revision: ${questionnaire.documentInfo?.revisionNo || "N/A"}`, {
                checklistId,
                operationRef,
                sequenceNumber: questionnaire.sequenceNumber,
            });

            // ==================== GENERATE FILE PATHS (CREATES FOLDERS) ====================
            const fileName = `${DOCUMENT_TYPE}-${questionnaire.sequenceNumber}.docx`;
            ...
            await generateOpsOfd001ADoc(questionnaire, fullPath);
