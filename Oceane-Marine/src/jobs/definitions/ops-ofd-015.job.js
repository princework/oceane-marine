import STSHourlyQuantityLog from "../../lib/mongodb/models/operation-sts-checklist/OPS-OFD-015.js";
import Operation from "../../lib/mongodb/models/sts-documentation/StsOperation.js";
import { generateOpsOfd015Doc } from "../services/pdf/OPS-OFD-015.js";
import { getPdfStoragePath } from "../services/pdf/pdfStorage.js";
import fs from "node:fs";
import path from "node:path";

const DOCUMENT_TYPE = "OPS-OFD-015";

export function defineOpsOfd015Job(agenda) {
    agenda.define("generate-ops-ofd-015", async (job) => {
        ...
        try {
            console.log("🚀 Starting OPS-OFD-015 document generation job", { checklistId, operationRef });
            ...
            // ==================== FETCH LOG ====================
            const log = await STSHourlyQuantityLog.findById(checklistId);
            if (!log) {
                throw new Error(`Hourly quantity log not found with ID: ${checklistId}`);
            }

            if (!log.sequenceNumber) {
                throw new Error("Hourly quantity log missing sequenceNumber");
            }

            console.log(`📄 Generating document with Revision: ${log.documentInfo?.revisionNo || "N/A"}`, {
                checklistId,
                operationRef,
            ...
            const fileName = `${DOCUMENT_TYPE}-${log.sequenceNumber}.docx`;
            ...
            await generateOpsOfd015Doc(log, fullPath);
