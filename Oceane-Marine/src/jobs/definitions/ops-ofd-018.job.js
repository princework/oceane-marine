import STSTimesheet from "../../lib/mongodb/models/operation-sts-checklist/OPS-OFD-018.js";
import Operation from "../../lib/mongodb/models/sts-documentation/StsOperation.js";
import { generateOpsOfd018Doc } from "../services/pdf/OPS-OFD-018.js";
import { getPdfStoragePath } from "../services/pdf/pdfStorage.js";
import fs from "node:fs";
import path from "node:path";

const DOCUMENT_TYPE = "OPS-OFD-018";

export function defineOpsOfd018Job(agenda) {
    agenda.define("generate-ops-ofd-018", async (job) => {
        ...
        try {
            console.log("🚀 Starting OPS-OFD-018 document generation job", { checklistId, operationRef });
            ...
            // ==================== FETCH TIMESHEET ====================
            const timesheet = await STSTimesheet.findById(checklistId);
            if (!timesheet) {
                throw new Error(`Timesheet not found with ID: ${checklistId}`);
            }

            if (!timesheet.sequenceNumber) {
                throw new Error("Timesheet missing sequenceNumber");
            }

            console.log(`📄 Generating document with Revision: ${timesheet.documentInfo?.revisionNo || "N/A"}`, {
                checklistId,
                operationRef,
            ...
            const fileName = `${DOCUMENT_TYPE}-${timesheet.sequenceNumber}.docx`;
            ...
            await generateOpsOfd018Doc(timesheet, fullPath);
