import MooringMastersJobReport from "../../lib/mongodb/models/operation-sts-checklist/OPS-OFD-009.js";
import Operation from "../../lib/mongodb/models/sts-documentation/StsOperation.js";
import { generateOpsOfd009Doc } from "../services/pdf/OPS-OFD-009.js";
import { getPdfStoragePath } from "../services/pdf/pdfStorage.js";
import fs from "node:fs";
import path from "node:path";

const DOCUMENT_TYPE = "OPS-OFD-009";

export function defineOpsOfd009Job(agenda) {
    agenda.define("generate-ops-ofd-009", async (job) => {
        ...
        try {
            console.log("🚀 Starting OPS-OFD-009 document generation job", { checklistId, operationRef });
            ...
            // ==================== FETCH REPORT ====================
            const report = await MooringMastersJobReport.findById(checklistId);
            if (!report) {
                throw new Error(`Job report not found with ID: ${checklistId}`);
            }

            if (!report.sequenceNumber) {
                throw new Error("Job report missing sequenceNumber");
            }

            console.log(`📄 Generating document with Revision: ${report.documentInfo?.revisionNo || "N/A"}`, {
                checklistId,
                operationRef,
            ...
            const fileName = `${DOCUMENT_TYPE}-${report.sequenceNumber}.docx`;
            ...
            await generateOpsOfd009Doc(report, fullPath);
