import MooringMasterExpenseSheet from "../../lib/mongodb/models/operation-sts-checklist/OPS-OFD-029.js";
import Operation from "../../lib/mongodb/models/sts-documentation/StsOperation.js";
import { generateOpsOfd029Doc } from "../services/pdf/OPS-OFD-029.js";
import { getPdfStoragePath } from "../services/pdf/pdfStorage.js";
import fs from "node:fs";
import path from "node:path";

const DOCUMENT_TYPE = "OPS-OFD-029";

export function defineOpsOfd029Job(agenda) {
    agenda.define("generate-ops-ofd-029", async (job) => {
        ...
        try {
            console.log("🚀 Starting OPS-OFD-029 document generation job", { checklistId, operationRef });
            ...
            // ==================== FETCH EXPENSE SHEET ====================
            const expenseSheet = await MooringMasterExpenseSheet.findById(checklistId);
            if (!expenseSheet) {
                throw new Error(`Expense sheet not found with ID: ${checklistId}`);
            }

            if (!expenseSheet.sequenceNumber) {
                throw new Error("Expense sheet missing sequenceNumber");
            }

            console.log(`📄 Generating document with Revision: ${expenseSheet.documentInfo?.revisionNo || "N/A"}`, {
                checklistId,
                operationRef,
                sequenceNumber: expenseSheet.sequenceNumber,
            });

            // ==================== GENERATE FILE PATHS (CREATES FOLDERS) ====================
            const fileName = `${DOCUMENT_TYPE}-${expenseSheet.sequenceNumber}.docx`;
            ...
            await generateOpsOfd029Doc(expenseSheet, fullPath);
