...
export function buildPoacPassportExpiryReminderEmail({
  ...
  const html = renderEmailShell({
    ...
    title: "Valid Passport expiring soon",
    preheader: `The Valid Passport for POAC ${name} is set to expire on ${when}.`,
    bodyHtml: `
        <p style="margin:0 0 16px;">Dear Team,</p>
        <p style="margin:0 0 18px;">
          This is a formal reminder that the <strong>Valid Passport</strong> for
...
export function buildPoacPassportExpiredNoticeEmail({
  ...
  const html = renderEmailShell({
    ...
    title: "Valid Passport has expired",
    preheader: `The Valid Passport for POAC ${name} expired on ${when}.`,
    bodyHtml: `
        <p style="margin:0 0 16px;">Dear Team,</p>
        <p style="margin:0 0 18px;">
          This is a formal notice that the <strong>Valid Passport</strong> for
          the POAC listed below has expired.
        </p>
        ${emailAlertBox("Expired", `Valid Passport for POAC ${escapeHtml(name)} (STS Service Provider: ${escapeHtml(provider)}) expired on ${escapeHtml(when)}.`, "danger")}
        <p style="margin:0 0 16px;">
          Please renew the passport and update the POAC Certification Matrix as soon as possible to avoid operational interruptions.
        </p>
        <p style="margin:0;">
