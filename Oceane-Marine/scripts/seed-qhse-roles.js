/**
 * One-time script to assign qhseRole to existing users.
 *
 * Usage:
 *   node scripts/seed-qhse-roles.js
 */
...
async function main() {
  ...
  const remaining = await users.updateMany(
    { qhseRole: { $exists: false } },
    { $set: { qhseRole: "viewer" } }
  );
  console.log(`Set ${remaining.modifiedCount} remaining users to 'viewer' (qhseRole)`);
