/**
 * Send sample POAC Valid Medicals emails via Resend (no MongoDB).
 *
 * Usage (from Oceane-Marine root):
 *   npm run test:poac-medicals-email
 *   npm run test:poac-medicals-email -- expired
 *
...
const {
  ...
  "../src/lib/services/email/templates/HR/poacMedicalsExpiryReminder.js"
);
...
const { subject, html, text } = isExpired
  ? buildPoacMedicalsExpiredNoticeEmail(emailParams)
  : buildPoacMedicalsExpiryReminderEmail(emailParams);

try {
  const data = await sendResendEmail({ to, subject, html, text });
  console.log(
    isExpired
      ? "OK — test POAC Valid Medicals EXPIRED notice sent to:"
      : "OK — test POAC Valid Medicals 5d reminder sent to:",
    to.join(", ")
  );
